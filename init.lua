...
require('lazy').setup({
    ...
    config = function()
      ...
      function _G.smart_esc()
        ...
        if vim.bo.filetype == 'toggleterm' then
          ...
          -- Fallback to standard Escape behavior (send actual Esc key)
          -- Using feedkeys ensures the event is forwarded as if typed
          vim.api.nvim_feedkeys(vim.api.nvim_replace_termcodes('<Esc>', true, false, true), 'n', true)
        end
      end

      -- Map Esc in Terminal mode ('t') and Normal mode ('n')
      -- Note: 't' mode requires <C-\><C-n> to run commands usually
      vim.keymap.set('t', '<Esc>', [[<C-\><C-n><Cmd>lua smart_esc()<CR>]], { noremap = true, silent = true })
      vim.keymap.set('n', '<Esc>', [[<Cmd>lua smart_esc()<CR>]], { noremap = true, silent = true })
    end,
  },
  {
    'nvim-java/nvim-java',
  ...
  -- LSP Plugins
  {
    ...
    config = function()
      ...
      local servers = {
        ...
        omnisharp = {},
        neocmakelsp = {
          cmd = { 'neocmakelsp', 'stdio' }, -- must point to the executable
          filetypes = { 'cmake' },
  ...
  { -- Highlight, edit, and navigate code
    'nvim-treesitter/nvim-treesitter',
    config = function()
      local filetypes = { 'bash', 'c', 'diff', 'html', 'lua', 'luadoc', 'markdown', 'markdown_inline', 'query', 'vim', 'vimdoc', 'c_sharp' }
...
-- Enable automatically for sh files
vim.api.nvim_create_autocmd('FileType', {
  pattern = 'sh',
  callback = function()
    vim.lsp.start {
